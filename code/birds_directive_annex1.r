...
## Read data
xin <- read.csv(here("data/birds_directive_annexi+gbif.csv"))

## Group by order, family, and species to get unique species
x_grouped <- xin %>% group_by(order, family, scientificName) %>%
  summarise()
x_grouped
dim(x_grouped)
...
# Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  annotate("text", x = rep(max(x_df$id), ), y = c(1, 10, 20), label = c("1", "10", "20") , color="grey", size=3, angle=0, fontface="bold", hjust=1) +
  ...
  coord_polar() + 
  labs(fill = "Order")+
  geom_text(data=label_data, aes(x=id, y=n+0.1, label=familyName, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=4, angle= label_data$angle, inherit.aes = FALSE ) +
  annotate("text", x = 1, y = -18, label = paste("Species:", unique_x_count), size = 7, fontface = "bold") +
  # Add base line information
  geom_segment(data=base_data, aes(x = start, y = -0.5, xend = end, yend = -0.5), colour = "black", alpha=0.8, size=0.6 , inherit.aes = FALSE )  #+
#geom_text(data=base_data, aes(x = title, y = -1, label=orderName), hjust=c(1,1,0,0), colour = "black", alpha=0.8, size=2, fontface="bold", inherit.aes = FALSE)

p

ggsave(
  here("output/figures/birds_directive_gbif_col4.pdf"),
  width = 12,
  height = 10,
  dpi = 600
)
